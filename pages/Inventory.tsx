...
const Inventory: React.FC<InventoryProps> = ({ toys, setToys, categories, setCategories }) => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const toyData: Toy = {
      ...
      price: Number(formData.price) || 0, // Garante que é número
      imageUrl: formData.imageUrl || '',
      size: formData.size || '',
      quantity: Number(formData.quantity) || 1, // Garante que é número
      description: formData.description || '',
      status: (formData.status as ToyStatus) || ToyStatus.AVAILABLE
    };

    // Atualiza o estado global. Se o seu App.tsx tiver o useEffect com setDoc, salvará no Firebase automaticamente.
    setToys(prev => {
        const exists = prev.find(t => t.id === toyData.id);
        if (exists) {
            return prev.map(t => t.id === toyData.id ? toyData : t);
        }
        return [...prev, toyData];
    });
  ...
  const handleDeleteToy = async (id: string) => {
    ...
    try {
        // Remove do estado local (o App.tsx cuidará da remoção no Firebase se estiver configurado para sincronizar)
        setToys(prev => prev.filter(t => t.id !== id));
        
        // Exclusão direta no Firebase por segurança
  ...
  const handleRemoveCategory = (cat: string) => {
    if (cat === 'Geral') return alert("Categoria padrão não pode ser removida.");
    ...
    <div className="space-y-10">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">Brinquedos, Animações e Equipamentos.</p>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[110] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[40px] p-8 shadow-2xl space-y-6">
            <div className="flex justify-between items-center">
              <h2 className="text-2xl font-black text-slate-800">Categorias</h2>
              <button onClick={() => setIsCatModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-600"><X /></button>
            </div>
            <div className="flex gap-2">
              <input value={newCatName} onChange={e=>setNewCatName(e.target.value)} placeholder="Ex: Infláveis, Animação..." className="flex-1 px-4 py-3 bg-slate-50 rounded-2xl font-bold border-0 focus:ring-2 focus:ring-blue-500/20 text-sm" />
              <button onClick={handleAddCategory} className="p-4 bg-blue-600 text-white rounded-2xl hover:bg-blue-700 transition-colors"><Plus size={20}/></button>
            </div>
            <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
              {categories.map(c => (
                <div key={c} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl group">
      ...
      {isModalOpen && isAdmin && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4 md:p-6">
          <form onSubmit={handleSave} className="bg-white w-full max-w-xl rounded-[40px] p-6 md:p-10 space-y-6 max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-black text-slate-800">{editingToy ? 'Editar Detalhes' : 'Novo Item'}</h2>
            ...
            <div className="space-y-6">
                <div className="w-full h-48 rounded-[32px] overflow-hidden bg-slate-50 border-2 border-dashed border-slate-200 relative group">
                    ...
                    <button type="button" onClick={()=>fileInputRef.current?.click()} className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white flex-col gap-2 font-black text-xs uppercase tracking-widest">
                        <Upload size={24}/> Selecionar Foto
                    </button>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                    <div className="space-y-1">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tamanho (ex: 3x3m)</label>
                ...
                <div className="space-y-1">
                    ...
                    <input type="number" step="0.01" className="w-full px-6 py-4 bg-blue-50 text-blue-700 rounded-2xl font-black text-xl focus:ring-2 focus:ring-blue-500/20 outline-none" value={formData.price} onChange={e=>setFormData({...formData, price: Number(e.target.value)})} />
                </div>
            </div>

            <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-3xl font-black uppercase tracking-widest shadow-xl shadow-blue-100 transition-all hover:bg-blue-700">
                <Save size={18} className="inline mr-2" /> Salvar Item
            </button>
          </form>
        </div>
      )}
